...
class MapGestion extends React.Component {
  ...
  moveOnArea(area, evt) {
    const coords = { x: evt.nativeEvent.layerX, y: evt.nativeEvent.layerY };
    this.setState({
      moveMsg: `Vous êtes à  ${area.shape} ${area.name
            ...
            <Card>
              <CardContent>
                ...
                <Button
                  ...
                  width="2em"
                  onClick={() => {
                    if (window.confirm(' Voulez-vous vraiment supprimer toutes les destinations ?')) { this.deletePoints(this.state.actualID) };
                  }}
                  variant="contained"
                  color="primary"
                  size="large"
